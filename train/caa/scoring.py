...
from behaviors import get_results_dir, ALL_BEHAVIORS, COORDINATE, CORRIGIBLE, HALLUCINATION, MYOPIC_REWARD, SURVIVAL_INSTINCT, SYCOPHANCY, REFUSAL, RESULTS_PATH, get_analysis_dir
import glob
import json
...
def scoring(behaviors=ALL_BEHAVIORS, custom_paths: dict[str, list[str]]=None, overwrite=False, do_printing=False):
    ...
    for behavior in behaviors:
        ...
        for file in open_ended_results:
            ...
            with open(os.path.join(save_dir, f"{file}.txt"), "w") as f:
                f.write(f"Average score for {file}: {scores}")
            if do_printing:
...
def main():
    ...
    parser.add_argument("--custom_paths", type=str, nargs="+", default=None)
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--do_printing", action="store_true")
    args = parser.parse_args()
    scoring(args.behaviors, args.custom_paths, args.overwrite, args.do_printing)
